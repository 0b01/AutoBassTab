...
def get_activation(audio, sr, model_capacity='full', center=True, step_size=10,
    ...
    level = np.sqrt(np.abs(np.mean(frames, axis=0)))
...
def predict(audio, sr, model_capacity='full',
    ...
    A 4-tuple consisting of:
        ...
        activation: np.ndarray [shape=(T, 360)]
            The raw activation matrix
    """
    activation, level = get_activation(audio, sr, model_capacity=model_capacity,
    ...
    return time, frequency, confidence, activation, level
...
def process_file(file, output=None, model_capacity='full', viterbi=False,
    ...
    time, frequency, confidence, activation, level = predict(
        audio, sr,
    ...
    f0_data = np.vstack([time, frequency, confidence, level]).transpose()
    np.savetxt(f0_file, f0_data, fmt=['%.3f', '%.3f', '%.6f', '%.6f'], delimiter=',',
               header='time,frequency,confidence,level', comments='')
    if verbose:
...
def get_notes(time, frequency, confidence, audio):
    # np.vstack(time, frequency, confidence, audio)
    pass